...
export const createCarListingLike = async (req, res, next) => {
  ...
  try {
    const likeExists = await CarListingLike.findOne({
      where: { carListingId: carListingId, userId: userId },
    });

    if (likeExists) return res.sendStatus(200);
